...
function OrderMinistryPopup({ isOpen, onClose, nsi, onSave, id, printDate, type, isLoading, ministries }) {
  ...
  setAddFullName("Приказ " + ministry + " от " + date + " г. № " + number + "" + edition + " «" + name + "»");
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name={`${type}-nsi-form-${id}`} action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">{`${type === "edit" ? "Редактирование " : "Добавление "}`}Приказа Министерства</h3>
          <ul className="nsi-popup__list-input">
          <li className="nsi-popup__item-input">
            <h5 className="nsi-popup__input-name">Министерство</h5>
            <div className="select-wrapper">
              <select           
              id={`${type}-nsi-input-ministry-${id}`}
              name={`${type}-nsi-input-ministry-${id}`}
              onChange={handleAddMinistry}
              defaultValue={addMinistryValue}
              required   
              >
                <option value="placeholder" disabled hidden>Выберите министерство</option>
                {
                  ministries.map((elem) => (
                    <option key={elem.id} value={elem.id}>{elem.short_nominative}</option>
                  ))
                }
              </select>
...
export default OrderMinistryPopup;