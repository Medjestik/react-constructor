...
function OrderPopup({ isOpen, onClose, nsi, onSave, id, printDate, type, isLoading }) {
  ...
  React.useEffect(() => {  
    ...
    setAddFullName("Порядок " + name + " " + approve + " от " + date + " г. № " + number + ")."); 
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name={`${type}-nsi-form-${id}`} action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">{`${type === "edit" ? "Редактирование " : "Добавление "}`}Порядка</h3>
