...
function ISOPopup({ isOpen, onClose, nsi, onSave, id, printDate, type, isLoading }) {
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name={`${type}-nsi-form-${id}`} action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">{`${type === "edit" ? "Редактирование " : "Добавление "}`}ISO</h3>
          <ul className="nsi-popup__list-input">
          <li className="nsi-popup__item-input">
            <h5 className="nsi-popup__input-name">Заглавие на русском языке</h5>
            <input 
            ...
            <span className={`nsi-popup__input-error ${addNameError ? "nsi-popup__input-error_type_show" : ""}`}>Заполните заглавие</span>
          </li>
          <li className="nsi-popup__item-input">
