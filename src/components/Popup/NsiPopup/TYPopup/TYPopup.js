...
function TYPopup({ isOpen, onClose, nsi, onSave, id, printDate, type, isLoading }) {
  ...
  React.useEffect(() => { 
    ...
    setAddFullName("ТУ "+ code + ". «" + name + "». " + res);
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name={`${type}-nsi-form-${id}`} action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">{`${type === "edit" ? "Редактирование " : "Добавление "}`}Технических условий</h3>
