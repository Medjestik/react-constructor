...
function RussiaCodexPopup({ isOpen, onClose, nsi, onSave, id, printDate, type, isLoading }) {
  ...
  React.useEffect(() => { 
    ...
    let name = addName.length > 0 ? addName : "<наименование кодекса>" 
     
    setAddFullName(name + " кодекс Российской Федерации от "+ date +" № " + number + " " + edition);
    // eslint-disable-next-line
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name={`${type}-nsi-form-${id}`} action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">{`${type === "edit" ? "Редактирование " : "Добавление "}`}кодекса Российской Федерации</h3>
...
export default RussiaCodexPopup;