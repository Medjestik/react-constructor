...
function GovernmentDecreePopup({ isOpen, onClose, nsi, onSave, id, printDate, type, isLoading }) {
  ...
  React.useEffect(() => {  
    ...
    setAddFullName("Постановление Правительства Российской Федерации от " + date + " г. № " + number + " " + edition + " «" + name + "»"); 
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name={`${type}-nsi-form-${id}`} action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">{`${type === "edit" ? "Редактирование " : "Добавление "}`}Постановления Правительства Российской Федерации</h3>
...
export default GovernmentDecreePopup;