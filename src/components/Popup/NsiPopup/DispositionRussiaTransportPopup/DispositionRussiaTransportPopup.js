...
function DispositionRussiaTransportPopup({ isOpen, onClose, emptyNsi, onAdd, id, printDate }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    const newNsi = { ...emptyNsi, nsiName: addName, nsiProtocolNumber: addProtocolNumber, nsiProtocolDate: addProtocolDate, nsiEdit: addEdition, type_id: id, nsiFullName: addFullName };
  ...
  setAddFullName("Распоряжение Минтранса России от " + date + " г. № " + number + "" + edition + " «" + name + "»");
  ...
  return (
    ...
    >
      <form className="popup__form popup__form_type_large" name="add-nsi-form" action="#" noValidate onSubmit={handleSubmit}>
          <h3 className="nsi-popup__title">Добавление Распоряжения Минтранса России</h3>
...
export default DispositionRussiaTransportPopup;