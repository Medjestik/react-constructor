...
import videoImg from '../../../images/achievement.png';
...
function MethodItem({ chapter }) {
  ...
  function defineImg(type) {
    switch(type) {
      case"video":
        return (<img className="method__parts-img" src={videoImg} alt="аватар"></img>);
      default:
        return (<img className="method__parts-img" src={videoImg} alt="аватар"></img>);
    }
  }
        ...
        <ul className="method__parts-list">
          {
            chapter.parts.map((part, i) => (
              <li key={i} className="method__parts-item">
                ...
                <a href={part.link} alt="ссылка на материал" rel="noreferrer" target="_blank">Перейти</a>
              </li>
            ))
          }
        </ul>
